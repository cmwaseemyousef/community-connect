...
app.get('/api/access-points', (req, res) => {
    ...
    database.all('SELECT * FROM access_points WHERE active = 1', (err, rows) => {
        if (err) {
...
app.get('/api/stats', (req, res) => {
    ...
    database.get('SELECT COUNT(*) as total FROM access_points WHERE active = 1', (err, row) => {
        if (err) {
            res.status(500).json({ error: err.message });
            return;
        }
        ...
        database.get('SELECT COUNT(*) as total FROM bookings WHERE end_time > datetime("now")', (err, row) => {
            if (err) {
                res.status(500).json({ error: err.message });
                return;
            }
            ...
            database.get('SELECT SUM(current_users) as total FROM access_points WHERE active = 1', (err, row) => {
                if (err) {
                    res.status(500).json({ error: err.message });
                    return;
                }
